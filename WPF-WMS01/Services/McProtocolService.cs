...
namespace WPF_WMS01.Services
{
    ...
    public class McProtocolService : IMcProtocolService
    {
        ...
        public async Task<ushort[]> ReadWordsAsync(string ipAddress, string deviceCode, int startAddress, ushort numberOfWords)
        {
            ...
            ushort dataLength = (ushort)(command.Length + 2); // +2 for monitoring timer value
        ...
        public async Task<bool> WriteWordsAsync(string ipAddress, string deviceCode, int startAddress, ushort[] values)
        {
            ...
            ushort dataLength = (ushort)(command.Length + 2); // +2 for completion code
            byte[] header = CreateMcProtocolHeader(dataLength);
            byte[] request = header.Concat(command).ToArray();
