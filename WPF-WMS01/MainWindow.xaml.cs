...
namespace WPF_WMS01
{
    public partial class MainWindow : Window
    {
        ...
        private void OutRackNoTextBox_PreviewTextInput(object sender, TextCompositionEventArgs e)
        {
            // 이 부분은 물리 키보드 입력을 막는 용도로 유지
            Regex regex = new Regex("[^0-9]+");
            e.Handled = regex.IsMatch(e.Text);
        ...
        private void OutRackNoTextBox_GotFocus(object sender, RoutedEventArgs e)
        {
            TextBox textBox = sender as TextBox;
            if (textBox != null)
            {
                ...
                // 팝업 이벤트 구독
                _currentKeypadPopup.KeypadTextChanged += Keypad_KeypadTextChanged3;
        ...
        private void OutRackNoTextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            // 관련된 논리적 포커스 이동인지 확인
            // 키패드 팝업이 TextBox의 "논리적" 자식(Logical Child)으로 간주되지 않기 때문에
            // LostFocus 이벤트가 발생할 수 있습니다.
        ...
        private void Keypad_KeypadTextChanged3(object sender, string newText)
        {
            // TextBox의 MaxLength를 고려하여 텍스트 업데이트
            if (OutRackNoTextBox != null)
            {
                if (newText.Length > OutRackNoTextBox.MaxLength)
                {
                    OutRackNoTextBox.Text = newText.Substring(0, OutRackNoTextBox.MaxLength);
                }
                else
                {
                    OutRackNoTextBox.Text = newText;
                }
                OutRackNoTextBox.CaretIndex = OutRackNoTextBox.Text.Length; // 커서를 맨 뒤로 이동
            }
        }
